...
-- 结论: 7已经达到最大选择性, 但选择性是动态变化的, 不能一概而论
alter table employees add key (first_name(7));
-- 局限性：无法做order by、group by; 也无法使用覆盖索引
-- "后缀索引"：额外创建一个字段，比如说first_name_reverse，在存储的时候，把first_name的值翻转过来再存储。
-- 比方说：Facello  ==> ollecaF存储到first_name_reverse, 最后再创建前缀索引, 因为mysql没有后缀索引